...
class Streamer:
    def __init__(self, model_size="base"):  # base model: better Hindi/Hinglish recognition
        self.model = None
    ...
    def listen(self):
        ...
        with sd.InputStream(samplerate=16000, channels=1, callback=self._audio_cb):
            while self.running:
                if self.q.qsize()*1024/16000 > 2.0:          # 2.0 s chunk – better for Hindi/Hinglish
                    if self.model is not None:
                        try:
                            chunk = np.concatenate([self.q.get() for _ in range(self.q.qsize())]).flatten()
                            result = self.model.transcribe(chunk)  # language=None → auto-detect Hindi/Hinglish/English
